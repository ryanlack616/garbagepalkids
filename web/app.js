...
(function () {
  ...
  function render() {
    ...
    // Apply image filter (raw/clean is the default display)
    filteredCards = cards.filter(c => {
      if (currentFilter === 'framed') return c.framed;
      if (currentFilter === 'raw') return c.raw;
      return true;
    });
  ...
  function createCardElement(card, index) {
    ...
    const imgSrc = card.raw || card.framed;
  ...
  function showCardInLightbox(card, index) {
    lightboxIndex = index;
    lightboxImg.src = card.raw || card.framed;
  ...
  function downloadCard(card) {
    const src = card.raw || card.framed;
    if (!src) return;
