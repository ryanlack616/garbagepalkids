...
(function () {
  ...
  function renderSeriesNav() {
    ...
    seriesNav.innerHTML = '<button class="series-chip active" data-series="all">All Series</button>';
    ...
    seriesList.forEach(name => {
      ...
      btn.innerHTML = `${meta.emoji} ${name} <span class="chip-count">${count}</span>`;
    ...
    // Re-bind "All"
    seriesNav.querySelector('[data-series="all"]').addEventListener('click', (e) => {
      document.querySelectorAll('.series-chip').forEach(b => b.classList.remove('active'));
  ...
  function renderSeriesInfo() {
    ...
    seriesGrid.querySelectorAll('[data-browse]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        currentSeries = name;
        currentPage = 1;
        // Activate the matching series chip
        document.querySelectorAll('.series-chip').forEach(b => {
          b.classList.toggle('active', b.dataset.series === name);
        });
        render();
        scrollToControls();
      });
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', () => {
      ...
      text += '\n\nhttps://rlv.lol/garbagepalkids/suggest.html';
